...
<div 
    x-data="{ 
        ...
        addToast(type, message) {
            const id = Date.now();
            this.toasts.push({ id, type, message, show: true, progress: 100 });
            const duration = 3000;
            const interval = 20;
    ...
    x-init="
        ...
        // Escuta eventos JavaScript customizados (fallback)
        window.addEventListener('toast-success', (e) => {
            addToast('success', e.detail?.message || e.detail);
        });
        window.addEventListener('toast-error', (e) => {
            addToast('error', e.detail?.message || e.detail);
        });
    "
        ...
        >
            <div 
                :class="toast.type === 'success' 
                    ? 'bg-gradient-to-r from-[#10b981] to-[#059669] border border-[#10b981]/20' 
                    : 'bg-gradient-to-r from-destructive to-[#dc2626] border border-destructive/20'"
                class="relative rounded-xl shadow-2xl p-5 flex items-start gap-4 overflow-hidden backdrop-blur-sm"
            >
                <!-- Efeito de brilho -->
                ...
                <div class="relative flex-1 min-w-0 pt-0.5">
                    <p 
                        :class="toast.type === 'success' ? 'text-white' : 'text-destructive-foreground'"
                ...
                <!-- Botão Fechar -->
                <button
                    @click="toast.show = false; setTimeout(() => toasts = toasts.filter(t => t.id !== toast.id), 200)"
                    :class="toast.type === 'success' ? 'text-white hover:bg-white/25' : 'text-destructive-foreground hover:bg-white/25'"
                    class="relative flex-shrink-0 w-8 h-8 rounded-lg transition-all duration-200 active:scale-95 flex items-center justify-center"
                >
