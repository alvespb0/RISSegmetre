<x-app-layout>
    <div class="p-6 space-y-6 max-w-2xl mx-auto">
        <div>
            ...
            <h2 class="text-2xl font-semibold text-foreground mt-2">Editar Integração</h2>
            <p class="text-muted-foreground">Atualize os dados da integração {{ $integracao->sistema }}</p>
        </div>

        <div class="bg-card border border-border rounded-lg p-6 max-w-2xl">
            <form action="{{ route('dev.integracoes.update', $integracao) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                @include('integracoes._form', ['integracao' => $integracao])
